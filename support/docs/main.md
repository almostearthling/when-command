...
**When** is an automation tool for desktop environments: it is implemented as a GUI application that offers the possibility to define certain conditions which, when satisfied, cause certaint tasks to be performed. Conditions can be of various kinds: time intervals or specific instants in time, session status, probing the current conditions of the system, events or signals triggered by the system or other applications.
...
The first is the main core that **When** uses to accomplish its mission: unlike previous version, **When** totally relies on **whenever** as its internal engine instead of implementing a scheduler on its own. The second can be considered as a complement to **When**, in the sense that **When** is designed to share its configuration base with **whenever_tray** in a way that would allow the latter to be launched as an alternative frontend for running **whenever** after having configured it with the help of **When**.
...
* ~~Toolbox~~
* Generated [Configiration Files](configfile.md)
...
For the sake of readability, a glossary follows for terms used throughout this documentation.
