...
class form_LuaScriptCondition(form_Condition):
    ...
    def __init__(self, tasks_available, item=None):
        ...
        sb_luaVars = ttk.Scrollbar(
            sftv_luaVars, orient=tk.VERTICAL, command=tv_luaVars.yview
        )
        ...
        b_addVar = ttk.Button(
            area, text=UI_UPDATE, width=BUTTON_STANDARD_WIDTH, command=self.add_var
        )
        b_delVar = ttk.Button(
            area, text=UI_DEL, width=BUTTON_STANDARD_WIDTH, command=self.del_var
        )
        ...
        sftv_luaVars.grid(
            row=11, column=0, columnspan=4, sticky=tk.NSEW, padx=PAD, pady=PAD
        )
