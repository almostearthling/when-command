...
There are also some configuration steps at system level that might have to be performed if filesystem monitoring does not work properly: when a watched directory is big enough, the default system watches may not be enough. There are many sources of information on how to increase the amount of system watches, and as usual StackExchange is one of the most valuables: see [Kernel inotify watch limit reached](http://unix.stackexchange.com/a/13757/125979) for a detailed description.
