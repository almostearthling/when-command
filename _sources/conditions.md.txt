...
It allows to set the mandatory item _Name_ (an alphanumeric string beginning either with a letter or an underscore[^1]), the tasks associated with it, and to decide other behaviors specific to conditions:
...
* _Activate further conditions_: conditions for which this option is checked will concur in activation of other specifically built [conditions](cond_confluence.md).
...
:::{caution}
When a condition is set to _activate further conditions_ by checking the appropriate box, it can only be used for the specific purpose: this means that no other _tasks_ can be triggered by this condition, and in fact possible _tasks_ that were listed as its consequence prior to checking the box are deleted, and the entire _tasks_ section in the form is disabled.
:::
...
* Conditions [_activated by other conditions_](cond_confluence.md)
